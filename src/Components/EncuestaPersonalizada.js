...
class EncuestaPersonalizada extends Component {
    ...
    componentDidMount() {
        ...
        db.collection('usuarios').doc(uid).collection('encuestas').get()
            .then(querySnapshot => {
                const encuestas = querySnapshot.docs.map(doc => doc.id); //NRC DE CADA CURSO
    ...
    onSave = event => {
        ...
        if (this.state.nombreEncuestas.includes(this.state.nombreEncuesta)) {
            this.setState({ error: 'Ya posees una ecnuesta con ese nombre' })
            setTimeout(() => {
                this.setState({ error: null });
            }, 3000);
        } else {
...
export default EncuestaPersonalizada;